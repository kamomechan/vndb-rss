...
const cache = {
  "/official": { data: "", expiresAt: 0 },
  "/unofficial": { data: "", expiresAt: 0 },
...
function generateLanguageLabel(path: PathType, languages: LanguageItem[]) {
...
function generateImageTags(images: ImageItem[]) {
  // narrow
  if (images.length === 0) {
    return "";
  }
...
function generateOPML() {
  const feeds = [
    { title: "Fan TL/民间汉化", xmlUrl: `${baseUrl}/unofficial` },
    { title: "Official TL/官方中文", xmlUrl: `${baseUrl}/official` },
  ...
  try {
    ...
    response.data.results.forEach((item: any) => {
      ...
      const customTitle = shouldUseAltTitle
        ? `${item.alttitle || item.title}` // The 'alttitle' may be null
        : `${item.title}`;
...
// Fan TL/民间汉化
app.get("/unofficial", async (req: Request, res: Response) => {
  try {
    const filters = [
      "and",
      [
    ...
    const rssXml = await generateRSS(
      ...
      filters,
      "Fan TL/民间汉化",
      "Fan translated visual novels/社区翻译的视觉小说"
    );
...
// Official TL/官方中文
app.get("/official", async (req: Request, res: Response) => {
  try {
    const filters = [
      "and",
      [
    ...
    const rssXml = await generateRSS(
      ...
      filters,
      "Official TL/官方中文",
      "Official English visual novels/官方中文视觉小说"
    );
...
app.listen(port, host, () => {
  ...
  console.log(`- Fan TL/民间汉化: http://${host}:${port}/unofficial`);
  console.log(`- Official TL/官方中文: http://${host}:${port}/official`);
